...
public class Database extends SQLiteOpenHelper {
    ...
    public Database() {
        super(MyApp.getContext(), "ProductsDatabase", null, 1); // ProductsDatabase = Database name, 1 = Database Version.
    }
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        db.execSQL("CREATE TABLE Products(_id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL, price REAL DEFAULT 0)");
    ...
    public void addProduct(Movie movie) {
        String sql = String.format("INSERT INTO Products(name,body) VALUES('%s',%s)", movie.getName(), movie.getBody());
    ...
    public void updateProduct(Movie movie) {
        String sql = String.format("UPDATE Products SET name='%s',body=%s WHERE _id=%d", movie.getName(), movie.getBody(), movie.getId());
    ...
    public void deleteProduct(Movie movie) {
        String sql = String.format("DELETE FROM Products WHERE _id=%d", movie.getId());
        SQLiteDatabase db = getWritableDatabase();
        db.execSQL(sql);
        db.close();
    }
    ...
    // Get all products:
    public ArrayList<Movie> getAllProducts(Context context) {
        ...
        while(cursor.moveToNext()) {
            ...
            Movie product = new Movie(context, name, body, id);
