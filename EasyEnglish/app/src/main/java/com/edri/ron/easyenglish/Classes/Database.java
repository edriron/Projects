...
public class Database extends SQLiteOpenHelper implements Serializable {
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        db.execSQL("CREATE TABLE Words(_id INTEGER PRIMARY KEY AUTOINCREMENT, word TEXT NOT NULL, trans TEXT NOT NULL, description TEXT NOT NULL, example TEXT NOT NULL, currects REAL DEFAULT 0, mistakes REAL DEFAULT 0, rating REAL DEFAULT 0, type REAL DEFAULT 0)");
    ...
    public void addWord(Word word) {
        String sql = String.format("INSERT INTO Words(word,trans,description,example,currects,mistakes,rating,type) VALUES('%s','%s','%s','%s',%d,%d,%d,%d)", word.getName(), word.getTrans(), word.getDescription(),
                word.getExample(), word.getCurrects(), word.getMistakes(), word.getRating(), word.getTypeByInt());
    ...
    public void updateWord(Word word) {
        String sql = String.format("UPDATE Words SET word='%s',trans='%s', description='%s', example='%s', currects=%d, mistakes=%d, rating=%d, type=%d WHERE _id=%d", word.getName(), word.getTrans(),
                word.getDescription(), word.getExample(), word.getCurrects(), word.getMistakes(), word.getRating(), word.getTypeByInt(), word.getId());
    ...
    public ArrayList<Word> getAllWords() {
        ...
        int currectsIndex = cursor.getColumnIndex("currects");
        int mistakesIndex = cursor.getColumnIndex("mistakes");
        ...
        while(cursor.moveToNext()) {
            ...
            int currects = cursor.getInt(currectsIndex);
            int mistakes = cursor.getInt(mistakesIndex);
            int rating = cursor.getInt(ratingIndex);
            int type = cursor.getInt(typeIndex);
            Word word = new Word(name, trans, description, example, currects, mistakes, rating, type, id);
            words.add(word);
        }
...
/*
    ...
    // Add a new product:
    public void addWord(Product product) {
    ...
    // Update an existing product:
    public void updateWord(Product product) {
    ...
    // Delete an existing product:
    public void deleteWord(Product product) {
        SQLiteDatabase db = getWritableDatabase();
    ...
    // Get all products:
    public ArrayList<Product> getAllWords() {
